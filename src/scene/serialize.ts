...
export type DrawCommand =
  ...
  | { cmd: 'draw_svg'; x: number; y: number; w: number; h: number; a: number; src: string }
...
function cmdSignature(c: Exclude<DrawCommand, { cmd: 'draw_image' }>): string {
  switch (c.cmd) {
    ...
    case 'draw_svg':
      return `svg:${c.x},${c.y},${c.w},${c.h},${c.a},${c.src}`;
    case 'overlay':
      return `o:${c.a}`;
  }
}
...
function emitNode(node: SceneNode, cmds: DrawCommand[], layout: ReadonlyMap<SceneNode, LayoutBox>, parentLb?: LayoutBox, offsetX = 0, alpha = 1): void {
  if (node.type === 'box') {
    ...
    // Inherited opacity: each node's own opacity multiplies the cascade coming
    // from its ancestors, and is in turn passed down to children — so fading a
    // wrapper fades its whole subtree, not just its own background fill.
    const a = (node.style?.opacity ?? 1) * alpha;
    ...
    if (node.style?.shadowColor && shadowOpacity > 0) {
      ...
      shadowCmd = {
        ...
        tl, tr, br, bl,
        r: sr, g: sg, b: sb, a: sa * shadowOpacity * a,
    ...
    if (!node.children.some(isAbsolute)) {
      for (const child of node.children) emitNode(child, cmds, layout, lb, childOffsetX, a);
    } else {
      for (const child of [...absNeg, ...flow, ...absPos]) emitNode(child, cmds, layout, lb, childOffsetX, a);
    }
    ...
    const a = ca * (node.style?.opacity ?? 1) * alpha;
    ...
    const a = (node.style?.opacity ?? 1) * alpha;
    if (a > 0.001) {
      cmds.push({ cmd: 'draw_svg', x: lb.x, y: lb.y, w: lb.w, h: lb.h, a, src: node.src });
    }
    ...
    const a = (node.style?.opacity ?? 1) * alpha;
    if (a > 0.001) {
      cmds.push({ cmd: 'draw_svg', x: lb.x, y: lb.y, w: lb.w, h: lb.h, a, src: svgNode._cachedSrc });
    }
  }
}
...
//  DRAW_SVG:    [1]=x [2]=y [3]=w [4]=h [5]=a  str0=src
